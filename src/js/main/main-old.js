...
import { diskWatcher, mapProject } from './contents/index.js'
...
if (!app.isPackaged) {
  ...
  console.log('// - - - - - - - - - - - - - - -')
  console.log(`Gambier. Chrome ${process.versions['chrome']}`)
  console.log('- - - - - - - - - - - - - - - //')
}
...
store.onDidAnyChange((state, oldState) => {
  const windows = BrowserWindow.getAllWindows()
  if (win !== undefined && windows.length) {
    windows.forEach((window) => window.webContents.send('stateChanged', state, oldState))
  }
})

// Set app startup conditions
// TODO: Temporarily setting projectPath here, manually, in code.
store.set('projectPath', '/Users/josh/Desktop/Test notes')
store.set('isColdStart', true)
...
ipcMain.on('saveWindowStateToDisk', (event, state) => {
  // store.store = state
})
