/*
Wizard is a singleton. As follows:
- WizardManager instantiates and exports wizard's state as Svelte store.
- Components called `setTarget` to update the store.
- Wizard.svelte imports store. When store changes, it updates itself.
*/
...
/**
 * Create instance of the Wizard store.
 * @returns - Instance of the Wizard store.
 */
  ...
  const methods = {
    ...
    close() {
      ...
      update(oldState => ({
        ...
        element: null,
        isVisible: false,
        suppressWarnings: false, // Reset to default
      }))
...
export function onHoverElementThatTriggersWizard(cm, el, metaKey, altKey) {
  ...
  if (showImmediately) {
    ...
    store.setTargetAfterTimer(cm, el, 1000, () => {
...
export async function onKeyDownCheckIfWeShouldOpenWizard(cm, evt) {
  ...
  if (isTimerActive) store.cancelTimer()
...
export function onKeyUpCheckIfWeShouldCloseWizard(cm, evt) {
  ...
  if (evt.key == 'Alt' && storeValue.openedBy.altKey) {
    // store.close()
    // Do nothing. Let wizard stay open.
  } else if (evt.key == 'Meta' && storeValue.openedBy.metaKey) {
    store.close()
  }
}
